function [s, cfg] = statfun_depsamplesT2(cfg, dat, design)
...
if strcmp(cfg.computestat,'yes')
  ...
  if any(isnan(diffmat(:)))
    ...
    for s = 1:nsmpls
        s.stat(s,1) = nunits * [real(avgdiff(s,:)) imag(avgdiff(s,:))] * ...
            inv(cov([real(diffmat(s,:))' imag(diffmat(s,:))'])) * [real(avgdiff(s,:)) imag(avgdiff(s,:))]';
    end
  else
    avgdiff = mean(diffmat,2);
    for s = 1:nsmpls
        s.stat(s,1) = (nunits-1) * (real(avgdiff(s,:)).^2 + imag(avgdiff(s,:)).^2) / ...
            sum((real(diffmat(s,:)) - real(avgdiff(s,:))).^2 + (imag(diffmat(s,:)) - imag(avgdiff(s,:))).^2);
    end
  end
end
